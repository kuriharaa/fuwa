...
client.on('message', async msg => { // eslint-disable-line
	...
	if (command === 'play') {
		...
		if (!voiceChannel) return msg.channel.send('enter voice channel');
		const permissions = voiceChannel.permissionsFor(msg.client.user);
		if (!permissions.has('CONNECT')) {
			return msg.channel.send('cant connect to voice channel, check permissions');
		}
		if (!permissions.has('SPEAK')) {
			return msg.channel.send('cant speak, check permissions');
		}
...
select number ranging from 1-10.
		...
		if (!msg.member.voiceChannel) return msg.channel.send('enter voice channel');
		...
		if (!msg.member.voiceChannel) return msg.channel.send('enter voice channel');
		if (!serverQueue) return msg.channel.send('no songs');
		if (!args[1]) return msg.channel.send(`volume is: **${serverQueue.volume}**`);
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
	...
	if (!serverQueue) {
		...
		try {
			...
			return msg.channel.send(`couldnt join voice channel: ${error}`);
		}
	} else {
